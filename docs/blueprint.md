...
| Importación de datos reales           | Utilidad propia en `lib/import.ts` (sin librería): carga automática de `public/dataset.json` al iniciar la aplicación, más un botón de importación manual en la landing page para reemplazar el dataset con otro archivo | Corresponde a la carga descrita en la sección 9. La automática puebla las 6 interfaces sin acción manual y sin depender del `localStorage` de un navegador en particular; la manual cubre traer datos nuevos sin tocar código ni recargar registro por registro. Ver sección 4.2.                                                                        |
...
cima-fix-research/
├── public/
│   └── dataset.json       # dataset real consolidado (sección 4.2)
├── src/
│   ├── components/
│   │   ├── ui/              # Button, Input, FormField, DataList, Modal, ExportButton, ImportButton...
...
│   │   ├── import.ts         # siembra localStorage desde un objeto dataset; usado por la carga automática (fetch de public/dataset.json) y por ImportButton (sección 4.2)
...
## 4.2 Carga de datos reales

La sección 9 ya establece que el líder consolida las entrevistas y observaciones reales y las carga en cada interfaz antes de la entrega final. Esta sección define los dos mecanismos disponibles para esa carga: automático al iniciar la aplicación, y manual mediante un botón en la landing page.

El dataset (en ambos mecanismos) usa siempre la misma forma: las claves de nivel superior son las que produce `createStorageKey()` (`lib/storage.ts`), y el valor de cada una es lo que esa interfaz espera leer con `getItem()`.

### Carga automática

- **`lib/import.ts`** hace `fetch` de `public/dataset.json` (versionado en el repositorio; justificación frente a `localStorage`: sección 3) al iniciar la aplicación, invocado desde `App.tsx` (Fase 5 de `scaffolding.md`) para que funcione con cualquier ruta de entrada, incluyendo deep links. Si el archivo no existe (404), no hace nada.
- Solo llama `setItem()` en claves donde `localStorage.getItem(clave)` sea `null` — nunca sobrescribe una clave con valor, incluyendo una lista vaciada intencionalmente (`[]`).

### Carga manual

- **`ImportButton`** (sección 6), un botón global en la landing page, permite seleccionar en cualquier momento un archivo con la misma forma que `dataset.json` y reutiliza la función de escritura de `lib/import.ts` (parametrizada para sobrescribir en este caso) — es el mecanismo para traer datos nuevos sin tocar código ni recargar registro por registro, y también para reflejar una actualización posterior del dataset consolidado por el líder (no existe una función de re-seed separada).
- A diferencia de la carga automática, **sobrescribe** las claves que el archivo trae, tengan o no valor previo; las claves que el archivo no incluye quedan intactas. Al no ser reversible, antes de escribir muestra una confirmación (`Modal`, sección 6) con las interfaces que se van a reemplazar.
- La validación de forma de cada registro es responsabilidad de quien genera el archivo, igual que en la carga automática.

Ambos mecanismos son, junto con la 4.1, las únicas excepciones al aislamiento por carpeta de la sección 4: escriben en el `localStorage` de las 6 interfaces, pero nunca en su código.
...
- `Modal` (confirmación de eliminar, confirmación de sobrescritura al importar, formularios de creación/edición)
- `ExportButton` (dispara `exportToJSON` / `exportToCSV` de `lib/export.ts`)
- `ImportButton` (landing page; permite seleccionar y cargar manualmente un archivo con la forma de `dataset.json`)
...
- Compartir con el líder la(s) clave(s) exacta(s) de `createStorageKey()` y la forma del registro correspondiente, para la carga de datos reales (sección 4.2)
